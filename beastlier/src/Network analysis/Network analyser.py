...
def analyse(headers, correctAnswers, guesses, outputFileName):
...
def mpcTree(guessFileName, data, burnin, mpcTreeName, correctAnswers, farms):
    ...
    for i in range(1,len(farms)):
        ...
        if correctAnswers[farms[i]]==currentBestNetworkMSPC[farms[i]]:
            mspcCorrectParents = mspcCorrectParents+1
    writeData.writerow(correctLine)       
    writeData.writerow(mpcBestGuessLine)
    writeData.writerow(mspcBestGuessLine)
    ...
    writeData.writerow(individualMultCredLine)
    writeData.writerow(individualAddCredLine)
    ...
    print "MSPC: Parent is correct in "+str(mspcCorrectParents)+" out of "+str(len(farms)-1)+" cases (" + str(round((mspcCorrectParents/(len(farms)-1))*100,1)) + "%)"
    print
...
def convert(oldFileName, newFileName):
        readData = csv.reader(open(oldFileName), dialect=csv.excel_tab)
        writeData = csv.writer(open(newFileName, 'wb'), dialect=csv.excel_tab)
        writeData.writerow(readData.next())
        writeData.writerow(readData.next())
        header = readData.next()
        newHeader = list()
        keepLength = int(((len(header)-1)/2)+1)
        for entry in header[0:keepLength]:
            if entry.endswith('infector'):
                newEntry = entry[:-9]
            else:
                newEntry = entry
            newHeader.append(newEntry)
        writeData.writerow(newHeader)
        currentLine = readData.next()
        while currentLine!=None:
            newLine = currentLine[0:keepLength] 
            writeData.writerow(newLine)         
            try:
                currentLine=readData.next()
            except StopIteration:
                currentLine=None
...
def main():            
    ...
    childFarmList = getFarmList(sys.argv[2], False)
    print "Reading list of parent farms"
    parentFarmList = getFarmList(sys.argv[2], True)
    print "Reading correct parents from tree simulation"
    correctParents = getCorrectParents(sys.argv[1])
#    print "Finding correct descendant sets"
#    correctDS = getCorrectDS(childFarmList, correctParents)
#    print "Finding correct subtrees"
#    correctSM = getCorrectSM(childFarmList, correctParents)
    print "Reading BEAST output"
    guesses = getGuesses(sys.argv[2], childFarmList, 1000000)
    print "Analysing parents (output to "+sys.argv[3]+")"
    analyse(parentFarmList, correctParents, guesses, sys.argv[3])
    print "Finding maximum parent credibility network (output to mpcTree.csv)"
    mpcTree(sys.argv[2], guesses, 1000000, "mpcTree.csv", correctParents, childFarmList)
#    print "Reading BEAST output"
#    guessesDS = getDSGuesses(sys.argv[2], childFarmList, 1000)
#    print "Analysing descendant sets (output to "+sys.argv[4]+")"
#    analyseDS(childFarmList, correctDS, guessesDS, sys.argv[4])
#    print "Finding maximum descendant set credibility network (output to mdsctree.csv)"
#    mdscTree(sys.argv[2], guessesDS, 1000000, "mdscTree.csv", correctParents)
    #===========================================================================
    # print "Reading BEAST output"
    # guessesSM = getSMGuesses(sys.argv[2], childFarmList, 1000)
    # print "Finding maximum subtree membership credibility tree (output to msmctree.csv)"
    # msmcTree(sys.argv[2], guessesSM, 0, "msmcTree.csv", correctParents)
    #===========================================================================
