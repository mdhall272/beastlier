 ...
 */
public class TransmissionSubtreeSlideA extends TreeOperator {
    ...
    public double proposal() {
        ...
        if (delta > 0) {
            ...
            if (PiP != null && PiP.getHeight() < newHeight) {
                ...
                final int possibleSources = intersectingEdges(tree, newChild, oldHeight,
                        iP.getPartitionElementNumber(), null);
        ...
        // 4 if we are sliding the subtree up.
            ...
            if (CiP.getHeight() > newHeight) {
                ...
                List<PartitionedTreeNode> newChildren = new ArrayList<PartitionedTreeNode>();
                final int possibleDestinations = intersectingEdges(tree, CiP, newHeight,
                        iP.getPartitionElementNumber(), newChildren);
    ...
    private int intersectingEdges(PartitionedTree tree, PartitionedTreeNode node, double height,
                                  int elementNo, List<PartitionedTreeNode> directChildren) {
        ...
        for (int i = 0; i < node.getChildCount(); i++) {
            count += intersectingEdges(tree, (PartitionedTreeNode)node.getChild(i), height, elementNo, directChildren);
        }
        return count;
    }
